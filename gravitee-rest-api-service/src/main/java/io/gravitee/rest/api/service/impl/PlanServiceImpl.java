...
import io.gravitee.definition.model.GraviteeDefinitionVersion;
...
public class PlanServiceImpl extends TransactionalService implements PlanService {
    ...
    public PlanEntity create(NewPlanEntity newPlan) {
        try {
            ...
            if (!GraviteeDefinitionVersion.V2.equals(api.getGraviteeDefinitionVersion())) {
            ...
            if (GraviteeDefinitionVersion.V2.equals(api.getGraviteeDefinitionVersion())) {
                ...
                io.gravitee.definition.model.Plan pl = new io.gravitee.definition.model.Plan();
                pl.setId(plan.getId());
                // TODO: transform path to flows
                pl.setFlows(new ArrayList<>());
                updateApi.setPlans(Collections.singletonList(pl));
    ...
    public PlanEntity update(UpdatePlanEntity updatePlan) {
        try {
            ...
            if (newPlan.getOrder() != updatePlan.getOrder()) {
                ...
                // Update plan in api definition
                ApiEntity api = apiService.findById(newPlan.getApi());
                api.getPlans().stream().forEach(plan -> {
                    if (plan.getId().equals(updatePlan.getId())) {
                        plan.setName(updatePlan.getName());
                    }
                });
                apiService.update(newPlan.getApi(), ApiService.convert(api));
    ...
    public void delete(String plan) {
        try {
            ...
            if (optPlan.get().getApi() != null) {
                ApiEntity api = apiService.findById(optPlan.get().getApi());
                List<io.gravitee.definition.model.Plan> plans = api.getPlans().stream()
                    .filter(plan1 -> !plan1.getId().equals(plan))
                    .collect(Collectors.toList());
                ...
                if (api.getPlans().size() != updateApiEntity.getPlans().size()) {
                    apiService.update(plan, updateApiEntity);
                }
            }
